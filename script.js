...
function updateFlags(result, carryFlag = false, overflowFlag = false, auxFlag = false) {
  ...
  registers.flags.AF = auxFlag ? 1 : 0;
}
...
// Check for auxiliary carry (carry from bit 3 to bit 4)
function checkAuxiliaryCarry(op1, op2, isAdd) {
  const mask = 0x0F; // Mask for lower nibble
  const op1Lower = op1 & mask;
  const op2Lower = op2 & mask;
  
  if (isAdd) {
    return (op1Lower + op2Lower) > mask;
  } else {
    return op1Lower < op2Lower;
  }
}
...
async function executeInstruction(instruction, labelMap) {
  ...
  // Get effective address for string operations
  function getEffectiveAddress(reg) {
    if (reg === 'si') return registers.si;
    if (reg === 'di') return registers.di;
    throw `Invalid register for string operation: ${reg}`;
  }

  // Update string index registers based on DF flag
  function updateStringIndex(reg, increment = 1) {
    const current = getRegister(reg);
    const newIndex = (current + increment) % 65536;
    setRegister(reg, newIndex);
  }

  // Execute repeated string operation
  async function executeRepeatedOperation(operation) {
    while (registers.cx > 0) {
      await operation();
      registers.cx--;
      updateUI(); // Update UI after each iteration
    ...
    switch (instr) {
      ...
      case 'sub': {
        ...
        updateFlags(
          ...
          checkAuxiliaryCarry(op1, value, false)  // Auxiliary Carry
        );
        break;
      }
      ...
      case 'rep': {
        if (args.length < 1) throw 'Missing argument for REP';
        const nextInstr = args[0].toLowerCase();

        let cx = getRegister('cx');
        if (cx <= 0) break;

        if (nextInstr === 'movsb') {
          while (cx > 0) {
            const sourceAddr = getEffectiveAddress('si');
            const destAddr = getEffectiveAddress('di');
            const value = memory[sourceAddr];
            memory[destAddr] = value;

            updateStringIndex('si', 1);
            updateStringIndex('di', 1);

            cx--;
          }
          setRegister('cx', 0); // after REP, CX becomes 0
        } 
        
        else if (nextInstr === 'movsw') {
          while (cx > 0) {
            const sourceAddr = getEffectiveAddress('si');
            const destAddr = getEffectiveAddress('di');
            const value = readWord(sourceAddr);
            writeWord(destAddr, value);

            updateStringIndex('si', 2);
            updateStringIndex('di', 2);
          ...
          throw `Unsupported REP instruction: ${nextInstr}`;
        }
      ...
      case 'movsb': {
        ...
        const value = memory[sourceAddr];
        memory[destAddr] = value;
        updateStringIndex('si');
        updateStringIndex('di');
        break;
      }
      ...
      case 'inc': {
        ...
        updateFlags(
          result,
          false,  // No carry for INC
          checkOverflow(op1, 1, result, true),  // Overflow
          checkAuxiliaryCarry(op1, 1, true)  // Auxiliary Carry
        );
        break;
      }
      ...
      case 'dec': {
        ...
        updateFlags(
          result,
          false,  // No carry for DEC
          checkOverflow(op1, 1, result, false),  // Overflow
          checkAuxiliaryCarry(op1, 1, false)  // Auxiliary Carry
        );
        break;
      }
